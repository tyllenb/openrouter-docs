...
OpenRouter provides you with a ton of open and closed source AI models to build with for text generation. Often times certain models perform better for certain tasks. OpenRouter lets you use each of these models and start building with the right model for your use case. 

### Prompting

Prompting is the essence of how these models work. You would provide the model with a prompt and in return the model will spit out an output based on the prompt provided. Creating great prompts will lead to better responses from your AI model. As such it is recommend that you follow best practices for prompt design as you work with these models.

## Models

There are various Large Language Models (LLM's) that OpenRouter provides to developers for their use. You can experiment with various models in the [chat playground](https://openrouter.ai/playground). These models are being constantly updated with the latest and best releases. If there is a model there that you want to use, but don't see, feel free to add it to the [OpenRouter Runner](www.runner.com).

To view which models are being used most, check out our usage information [here](https://openrouter.ai/rankings).

## Building with Text Completion Models

Choose the language or tool you want to get started with using the OpenRouter API for the Text Completions models. 
